...
class antithesis_fallback_sdk():
    ...
    def __init__(self):
        if 'ANTITHESIS_OUTPUT_DIR' not in os.environ:
            raise Exception(f"The environment variable ANTITHESIS_OUTPUT_DIR is not defined")
    ...
    def _declare(self, assert_type:str, id: str, message:str) -> dict:
        ...
        if assert_type not in self.assertion_types:
            raise Exception(f"The assertion type must be one of {', '.join(self.assertion_types)}")

        assertions_declare = self._get_assertion_declare()
        payload = {
            'antithesis_assert': assertions_declare[assert_type]
        }
    ...
    def do_assert(self, assert_type:str, id: str, message:str, condition:bool, details:dict = {}) -> None:
        '''
    ...
    def always(self, declare:bool, id:str, message:str, condition:bool = False, details:dict = {}, ):
        if declare:
            self.declare('always', id, message)
        else:
            self.do_assert('always', id, message, condition, details)

    def sometimes(self, declare:bool, id:str, message:str, condition:bool = False, details:dict = {}):
        if declare:
            self.declare('sometimes', id, message)
        else:
            self.do_assert('sometimes', id, message, condition, details)

    def reachable(self, declare:bool, id:str, message:str, condition:bool = False, details:dict = {}):
        if declare:
            self.declare('reachable', id, message)
        else:
            self.do_assert('reachable', id, message, condition, details)

    def unreachable(self, declare:bool, id:str, message:str, condition:bool = False, details:dict = {}):
        if declare:
